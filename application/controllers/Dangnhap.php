...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->data['title']='DanhVinhShop - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->email->to($email);
            $this->email->subject('Hệ thống DanhVinhShop - Quà thành viên mới');
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng DanhVinhShop, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : '.$tempcoupon.' , Mã này có giá trị tới ngày '.$tempdatelimit.'
        ...
        $this->data['title']='DanhVinhShop - Đăng ký tài khoản';   
...
//laylaipass
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['validation'] = TRUE;   
            $this->email->initialize($config);
            $this->email->from('DanhVinhShop', 'DanhVinhShop');
            $this->email->to($list['email']);
            $this->email->subject('DanhVinhShop - Lấy lại mật khẩu');
        ...
        $this->data['title']='DanhVinhShop - Quên mật khẩu';   
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='DanhVinhShop - Cập nhật mật khẩu mới';
